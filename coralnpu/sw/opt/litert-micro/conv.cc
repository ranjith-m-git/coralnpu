...
void ConvPerChannel(const ConvParams& params, const OpDataConvCustom& data,
  ...
  if (filter_height == 4 && filter_width == 4 && (input_depth % 4 == 0) &&
    ...
    // fallback print with details
    // 1. Safely pull structural metadata directly from the function parameters
    int input_dims_count = (input_shape.DimensionsCount() >= 4) ? input_shape.Dims(3) : 0;
    int output_dims_count = (output_shape.DimensionsCount() >= 4) ? output_shape.Dims(0) : 0;
    
    // 2. Determine the exact operator name variant based on layer dimensions
    const char* target_op_name = "CONV_2D";
    if (filter_height == 1 && filter_width == 1) {
        target_op_name = "CONV_2D (1x1 Pointwise)";
    } else if (input_dims_count == 3) {
        target_op_name = "CONV_2D (3-Channel Input Layer)";
    }
    
    // 3. Format and print the self-contained message with the operator name
    MicroPrintf("Fallback kernel: fh=%d fw=%d id=%d od=%d | Op: %s | Shapes: InCh=%d OutCh=%d", 
                filter_height, filter_width, input_depth, output_depth,
                target_op_name, input_dims_count, output_dims_count);
